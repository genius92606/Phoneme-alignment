...
def check_file(path):
    ...
    if os.path.exists(path) and os.path.isfile(path):
        ...
        if __name__ == "__main__":
            ...
            newpath = input("Please enter correct name or path for file, or type [q] to quit:  ")
    ...
    if not ((len(p) == 3 and p[-1] in ['0', '1', '2'] and p[:-1] in VOWELS) or (len(p) <= 2 and p in CONSONANTS)):
        ...
        if __name__ == "__main__":
            if len(p) == 2 and p in VOWELS:
                ...
                new_p = input("Please re-enter vowel transcription, or type [q] to quit:  ")
            else:
                print("Unknown phone %s (at position %i) in word %s!\n" % (p, i+1, w))
                new_p = input("Please correct your transcription for this phone, or type [q] to quit:  ")
            ## EMERGENCY EXIT:
...
def check_transcription(w):
    ...
    if len(w) > 3 and len(phones) < 2:
        ...
        new_trans = input("Please enter new transcription:  ")
            ...
            if len(entries) == 4:
                ...
                if __name__ == "__main__":
                    ...
                    for i in range(len(entries)):
                        print(i, "\t", entries[i])
                    stop_program = input("Stop program?  [y/n]")
                    if stop_program == "y":
...
def check_tempdir(tempdir):
    ...
    if os.path.isdir(tempdir):
        ...
        if len(contents) != 0 and not options.noprompt:
            ...
            print("(Files in directory:  %s )" % contents)
            overwrite = input("Overwrite and continue?  [y/n]")
            if overwrite == "y":
                ## delete contents of tempdir
                for item in contents:
                    os.remove(os.path.join(tempdir, item))
...
def read_transcription_file(trsfile):
    ...
    try:  ## try UTF-16 encoding first
        ...
        print("Try Encoding UTF-16!")
        lines = t.readlines()
    except UnicodeError:
        try:  ## then UTF-8...
            t = codecs.open(trsfile, 'rU', encoding='utf-8')
            print("Encoding is not UTF-16!")
            ...
            try:  ## then Windows encoding...
                ...
                print("Encoding is not UTF-8!")
                print("Try Encoding Windows-1252!")
                lines = t.readlines()
            except UnicodeError:
                t = open(trsfile, 'rU')
...
def FAAValign(opts, args, FADIR='', SOXPATH=''):
    ...
    for (text, line) in zip(trans_lines, all_input):
        ...
        ## normal tiers:
        #speaker = entries[1].strip().encode('ascii', 'ignore').replace('/', ' ')
    ...
    [starttime, stoptime, phonemes, stress]=main_textgrid.write_modified(tgfile)
    if options.verbose:
    ...
    return [starttime,stoptime,phonemes,stress]
